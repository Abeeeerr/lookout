...
/** Classic six-color (rainbow) Apple mark, rendered as crisp SVG.
 *  `mono` renders a single-color silhouette instead of the stripes. */
const APPLE_PATH =
...
export function AppleLogo({
  ...
  size?: number;
  mono?: string; // single color; omit for rainbow
  title?: string;
}) {
  // Unique per instance so multiple logos don't collide on the clipPath id.
  const id = `apple-clip-${useId()}`;
  return (
    <svg
      width={size}
      height={size}
